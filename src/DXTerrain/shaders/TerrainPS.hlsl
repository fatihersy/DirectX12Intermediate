...
float4 PS_Terrain(PSInput input) : SV_TARGET
{
    ...
    float wSand  = saturate(1.0f - height01 * 8.0f);
    float wGrass = saturate(1.0f - height01 * 2.2f) * saturate(1.0f - slope * 2.0f);
    float wRock  = saturate(slope * 2.5f);
    float wSnow  = saturate((height01 - 0.68f) * 5.0f) * saturate(1.0f - slope);
    ...
    float3 sandTex  = splat3.Sample(linearWrapSampler, tiledUV).rgb;
    float3 grassTex = splat0.Sample(linearWrapSampler, tiledUV).rgb;
    float3 rockTex  = splat1.Sample(linearWrapSampler, tiledUV).rgb;
    float3 snowTex  = splat2.Sample(linearWrapSampler, tiledUV).rgb;

    float3 sandTint  = float3(0.72f, 0.62f, 0.42f);
    float3 grassTint = float3(0.22f, 0.46f, 0.18f);
    float3 rockTint  = float3(0.42f, 0.40f, 0.38f);
    float3 snowTint  = float3(0.86f, 0.90f, 0.92f);
