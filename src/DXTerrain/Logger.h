...
inline void g_FDebug(const char* fmt, Args&&... args) {
    g_PlatformConsoleWrite(FlogLevel::FLOG_DEBUG, NSTool::format(fmt, std::forward<Args>(args)...));
}

template<typename... Args>
inline void g_FError(const char* fmt, Args&&... args) {
    g_PlatformConsoleWrite(FlogLevel::FLOG_ERROR, NSTool::format(fmt, std::forward<Args>(args)...));
}

template<typename... Args>
inline void g_FWarn(const char* fmt, Args&&... args) {
    g_PlatformConsoleWrite(FlogLevel::FLOG_WARN, NSTool::format(fmt, std::forward<Args>(args)...));
}
