...
{
    std::string message = std::format("HRESULT of 0x{:08X}", static_cast<unsigned int>(hr));

    LPSTR sysMsgBuf = nullptr;
    DWORD sysMsgLen = FormatMessageA(
        FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
        nullptr,
        static_cast<DWORD>(hr),
        MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
        reinterpret_cast<LPSTR>(&sysMsgBuf),
        0,
        nullptr
    );

    if (sysMsgLen != 0 and sysMsgBuf != nullptr)
    {
        std::string_view sysMsg(sysMsgBuf, sysMsgLen);
        while (not sysMsg.empty() and (sysMsg.back() == '\r' or sysMsg.back() == '\n')) sysMsg.remove_suffix(1);

        message += std::format(": {}", sysMsg);
    }

    if (sysMsgBuf != nullptr) LocalFree(sysMsgBuf);
...
inline void PrintDxgiDebugMessages()
{
    Microsoft::WRL::ComPtr<IDXGIInfoQueue> dxgiInfoQueue;
    if (FAILED(DXGIGetDebugInterface1(0, IID_PPV_ARGS(&dxgiInfoQueue)))) return;

    const UINT64 numMessages = dxgiInfoQueue->GetNumStoredMessages(DXGI_DEBUG_ALL);
    for (UINT64 i{}; i < numMessages; i++)
    {
        SIZE_T messageLength{};
        dxgiInfoQueue->GetMessage(DXGI_DEBUG_ALL, i, nullptr, &messageLength);

        std::vector<std::byte> messageData(messageLength);
        ...
        g_FError(std::string(message->pDescription, message->DescriptionByteLength));
    }
