...
ConstantAllocator::ConstantAllocator(ID3D12Device14* device, size_t totalSize, UINT frameInFlight)
{
    m_blobTotalSize = totalSize;
    m_blobFrameSize = totalSize / frameInFlight;
    ...
    device->CreateCommittedResource(
        &props, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE,
        &desc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_bufferDefault)
    );
...
{
    ASSERT(m_cpuAddr != nullptr and m_bufferDefault != nullptr);
